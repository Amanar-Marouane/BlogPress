...
$sql = "SELECT * FROM articles ORDER BY views_number DESC, likes_number DESC LIMIT 1;";
$peak_post = $conn->query($sql);
if ($peak_post->num_rows > 0) {
    while ($r = $peak_post->fetch_assoc()) {
        $GLOBALS['mochahadat'] = $r['views_number'];
        $GLOBALS['i3jabat'] = $r['likes_number'];
        $GLOBALS['sani3'] = $r['author_id'];
        $GLOBALS['content'] = $r['arti_desc'];
        $GLOBALS['3onwan'] = $r['title'];
        $sql = "SELECT user_name FROM users WHERE user_id = {$GLOBALS['sani3']}";
        $sani3_name = $conn->query($sql);
        if ($sani3_name->num_rows > 0) {
            $res = $sani3_name->fetch_assoc();
            $GLOBALS['sani3_name'] = $res['user_name'];
        }
        $sql = "SELECT COUNT(*) AS comm_num FROM comments JOIN articles ON comments.article_id = articles.article_id WHERE comments.article_id = {$GLOBALS['sani3']};";
        $RES = $conn->query($sql);
        if ($RES->num_rows > 0) {
            $comm_num = $RES->fetch_assoc();
            $GLOBALS['comm_num'] = $comm_num['comm_num'];
        }
    }
}
?>
...
<body class="HomeBody">
    <main>
        <section class="post-of-the-week">
            <h2>Post of the Week</h2>
            <a href='./article_page.php?article_id=<?php echo $GLOBALS['sani3']; ?>'>
                <div class="post-container">
                    <div class="post">
                        <div class="post-title">
                            <h3><?php echo $GLOBALS['3onwan']; ?></h3>
                        </div>
                        <div class="post-author">
                            <p>by <span><?php echo $GLOBALS['sani3_name']; ?></span></p>
                        </div>
                        <div class="post-snippet">
                            <p><?php echo $GLOBALS['content']; ?></p>
                        </div>
                        <div class="post-stats">
                            <div class="stat">
                                ...
                                <span><?php echo $GLOBALS['i3jabat']; ?></span>
                            </div>
                            <div class="stat">
                                <svg width='20px' height='20px' viewBox='0 -0.5 20 20' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'>
                                ...
                                <span><?php echo $GLOBALS['mochahadat']; ?></span>
                            </div>
                            <div class="stat">
                                ...
                                <span><?php echo $GLOBALS['comm_num']; ?></span>
                            </div>
                        </div>
                    </div>
                </div>
